    ...
    <div class="space-y-4">
        <div class="card-surface p-4 sm:p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <form action="{{ route('admin.users.index') }}" method="get" class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:w-[26rem]">
                    <label for="users-search" class="sr-only">{{ __('admin.search') }}</label>
                    <input
                        id="users-search"
                        type="text"
                        name="q"
                ...
                <article class="card-surface p-4 sm:p-5">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0 flex items-start gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-charcoal/45">#{{ $user->id }}</p>
                                <h3 class="mt-1 truncate text-base font-bold text-charcoal">{{ $user->name }}</h3>
                                <p class="mt-1 truncate text-sm text-charcoal/70" dir="ltr">{{ $user->email }}</p>
                            </div>
                        </div>

                        <span class="inline-flex shrink-0 items-center rounded-full border px-3 py-1 text-xs font-semibold {{ $roleClass }}">
                            {{ $primaryRole ? str($primaryRole)->headline() : '-' }}
                        </span>
                    </div>
